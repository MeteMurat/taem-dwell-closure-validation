...
def maybe_rad_to_deg_value(x):
    ...
    # AÃ§Ä± radyan gibi gÃ¶rÃ¼nÃ¼yorsa dereceye Ã§evir.
    if abs(x) <= 6.5:
        return np.degrees(x)
...
def pick_event_row(g):
    """
    Dwell-confirmed strict TAEM event row seÃ§imi.
    ...
    Not:
    EÄŸer end_reason kolonu varsa fakat taem_dwell_reached yoksa,
...
def get_delta_psi_deg(row):
    # Ã–nce doÄŸrudan delta_psi kolonunu kullan.
...
def main():
    ...
    for f in files:
        ...
        for vid, g in df.groupby(idcol):
            ...
            # Close-pass escape olanlarÄ± strict angular diagnostic tablosuna dahil etme.

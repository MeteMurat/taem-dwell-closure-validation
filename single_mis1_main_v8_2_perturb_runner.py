# -*- coding: utf-8 -*-
"""single_mis1_main_v8_2_perturb_runner.py

Run V8.2 final-success-flags with small mis1 initial-condition perturbations.
...
def _pick_case(index: int):
    ...
    if sp is not None:
        try:
            return copy.deepcopy(sp[int(index)])
        except Exception:
            pass
    cases = getattr(glbs, "cases", None)
    if cases is not None:
        try:
            return copy.deepcopy(cases[int(index)])
        except Exception:
            pass
    raise RuntimeError("multiset.StatusParams veya multiset.cases içinde mis_index bulunamadı.")


def _apply_perturbations(case: dict, heading_deg: float = 0.0, height_delta: float = 0.0, velocity_delta: float = 0.0):
    """Apply initial-condition perturbations to the selected case only."""
    init = case["MissileInitStatus"]
    if abs(float(heading_deg)) > 0.0:
        init["heading_angle"] = float(init["heading_angle"]) + math.radians(float(heading_deg))
    if abs(float(height_delta)) > 0.0:
        init["height"] = float(init["height"]) + float(height_delta)
    if abs(float(velocity_delta)) > 0.0:
        init["velocity"] = float(init["velocity"]) + float(velocity_delta)
    return case
...
def main():
    ...
    parser.add_argument("--heading-deg", type=float, default=0.0, help="Initial heading perturbation in degrees")
    parser.add_argument("--height-delta", type=float, default=0.0, help="Initial height perturbation in meters")
    parser.add_argument("--velocity-delta", type=float, default=0.0, help="Initial velocity perturbation in m/s")
    parser.add_argument("--out", default=os.path.join("store", "data_saved", "mis1_state_first_v8_2_perturb.csv"))
    args = parser.parse_args()

    case = _pick_case(args.mis_index)
    case = _apply_perturbations(case, args.heading_deg, args.height_delta, args.velocity_delta)
    ...
    result["perturb_heading_deg"] = float(args.heading_deg)
    result["perturb_height_m"] = float(args.height_delta)
    result["perturb_velocity_mps"] = float(args.velocity_delta)
    ...
    print(f"[PERT] heading_deg={args.heading_deg}, height_delta={args.height_delta}, velocity_delta={args.velocity_delta}")

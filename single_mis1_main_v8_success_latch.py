# -*- coding: utf-8 -*-
"""single_mis1_main_v8_success_latch.py

Run the success-first V8 recovery path.

This intentionally returns to the V1 dynamics/control law, because V1 was the
only family that produced a real TAEM dwell. V8 adds only clean success latch
...
def _load_guidance_class():
    module_name = "multiMissileGuideInstance_mis1_state_first_v8_success_latch"
    try:
        mod = __import__(module_name)
        return mod.Mis1StateFirstGuidanceV8Success
    except Exception:
        pass
...
Mis1StateFirstGuidanceV8Success = _load_guidance_class()
...
def main():
    ...
    parser.add_argument("--out", default=os.path.join("store", "data_saved", "mis1_state_first_v8_success_latch.csv"))
    ...
    guide = Mis1StateFirstGuidanceV8Success(case=case, vehicle_id=args.mis_index)
    ...
    if len(result):
        cols = [c for c in ["taem_reached", "taem_success_latched", "end_guide", "end_reason", "taem_dwell_s", "taem_h_err", "taem_v_err", "taem_s_go_err"] if c in result.columns]

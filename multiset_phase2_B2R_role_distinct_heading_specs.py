# -*- coding: utf-8 -*-
"""
Phase 2 / B2R role-distinct exact-single multiset.

Purpose
-------
B2 produced 3/3 PASS_STRICT, but numeric audit showed that mis0/mis1/mis2
were dynamically identical. This B2R file forces a real role distinction by
using the same target and V8.2 terminal/event definition, while assigning
small deterministic initial-heading offsets and role-specific PathSpec values.

This is still classification only: no new guidance law and no tuning during B2R.
The objective is to determine whether the exact-single orchestration pipeline
can produce distinct trajectories and how each role classifies under the same
TAEM checker.
"""
...
# Simulation controls
# ------------------------------------------------------------
MAXITER = int(float(os.environ.get("B2R_MAXITER", "2600000")))
t0 = 0
MIN_H = float(os.environ.get("B2R_MIN_H", "1e-2"))
GROUND_H = float(os.environ.get("B2R_GROUND_H", "5.0"))
INI_STEP = float(os.environ.get("B2R_INI_STEP", "0.1"))
...
def _fail(msg: str):
    raise RuntimeError("[B2R multiset] " + msg)
...
def _target_from_df_or_env(df: pd.DataFrame, row: pd.Series):
    ...
    lon_env = _env_angle_rad("B2R_TARGET_LON_DEG", "B2R_TARGET_LON_RAD")
    lat_env = _env_angle_rad("B2R_TARGET_LAT_DEG", "B2R_TARGET_LAT_RAD")
    if lon_env is not None and lat_env is not None:
        return lon_env, lat_env, "environment target"

    _fail("Could not infer target lon/lat from CSV. Set B2R_TARGET_LON_DEG/LAT_DEG.")
...
# ------------------------------------------------------------
# Role specs: force trajectory distinction with small heading offsets.
# These offsets are classification probes; do not tune within B2R.
# ------------------------------------------------------------
_ROLE_SPECS = [
    dict(
        vehicle_id=0,
        role_name="B2R_mis0_left_heading_probe",
        phase2_role="B2R_CLASSIFY_MIS0_LEFT_HEADING_PROBE",
        heading_delta_deg=-2.0,
        pathspec=dict(
            name="B2R_mis0_left_fixed_anchor_probe",
    ...
    dict(
        vehicle_id=1,
        role_name="B2R_mis1_nominal_B1E_control",
        phase2_role="B2R_REFERENCE_MIS1_B1E_CONTROL",
        heading_delta_deg=0.0,
        pathspec=dict(
            name="B2R_mis1_v8_2_validated_nominal",
    ...
    dict(
        vehicle_id=2,
        role_name="B2R_mis2_right_heading_probe",
        phase2_role="B2R_CLASSIFY_MIS2_RIGHT_HEADING_PROBE",
        heading_delta_deg=+2.0,
        pathspec=dict(
            name="B2R_mis2_right_case2_probe",
            policy_family="merged_winner_case4_case2",
            psi_bias_deg=-0.5,
            psi_bias_sgo_on_m=7.0e6,
...
def _make_case(spec: dict):
    ...
    pathspec.update(
        ...
        b2r_vehicle_id=int(spec["vehicle_id"]),
    )
    ...
    return {
        ...
        'MissileEndStatus': {
            ...
            "s": _STAEM,
            "heading_angle": None,
            "t": float(os.environ.get("B2R_END_T", "2150")),
        },
        'PathSpec': pathspec,
...
cases = StatusParams
case_num = int(float(os.environ.get("B2R_DEFAULT_CASE_NUM", "1")))
...
B2R_VEHICLE_IDS = [0, 1, 2]
B2R_BASELINE_CSV_USED = BASELINE_CSV
B2R_TARGET_SOURCE_USED = _TARGET_SOURCE
B2R_ROLE_SPECS = _ROLE_SPECS
...
MAX_EBR2_ITER = int(float(os.environ.get("B2R_MAX_EBR2_ITER", "5")))
STORE_DATA = os.environ.get("B2R_STORE_DATA", os.path.join("store", "data_saved", "phase2_B2R_role_distinct_combined.csv"))

print("[B2R multiset] baseline_csv =", BASELINE_CSV)
print("[B2R multiset] target lon/lat deg =", np.rad2deg(_TAR_LON), np.rad2deg(_TAR_LAT), "source=", _TARGET_SOURCE)
print("[B2R multiset] base init lon/lat/heading deg =", np.rad2deg(_INIT_LON), np.rad2deg(_INIT_LAT), np.rad2deg(_INIT_PSI_BASE))
print("[B2R multiset] end h/v/s =", _HTAEM, _VTAEM, _STAEM)
for _i, _case in enumerate(StatusParams):
    _ps = _case["PathSpec"]
    print(
        f"[B2R multiset] StatusParams[{_i}] role={_case.get('Phase2Role')} "

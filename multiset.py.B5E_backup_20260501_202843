# -*- coding: utf-8 -*-
"""
Phase 2 / B5A deterministic dispersion-grid multiset.

Purpose
-------
B5A extends the B2R/B5A role-distinct exact-single result to a deterministic
height/velocity perturbation grid. The default heading spread is fixed at [-2,0,+2] deg,
while common initial height and velocity perturbations are swept by the orchestrator.

The active perturbation case is read from environment variables:
    B5A_HEADING_SPREAD_DEG
    B5A_HEIGHT_DELTA_M
    B5A_VELOCITY_DELTA_MPS
The role headings are:
...
# Simulation controls
# ------------------------------------------------------------
MAXITER = int(float(os.environ.get("B5A_MAXITER", os.environ.get("B2R_MAXITER", "2600000"))))
t0 = 0
MIN_H = float(os.environ.get("B5A_MIN_H", os.environ.get("B2R_MIN_H", "1e-2")))
GROUND_H = float(os.environ.get("B5A_GROUND_H", os.environ.get("B2R_GROUND_H", "5.0")))
INI_STEP = float(os.environ.get("B5A_INI_STEP", os.environ.get("B2R_INI_STEP", "0.1")))
...
B5A_HEADING_SPREAD_DEG = float(os.environ.get("B5A_HEADING_SPREAD_DEG", "2.0"))
B5A_HEIGHT_DELTA_M = float(os.environ.get("B5A_HEIGHT_DELTA_M", "0.0"))
B5A_VELOCITY_DELTA_MPS = float(os.environ.get("B5A_VELOCITY_DELTA_MPS", "0.0"))
B5A_CASE_LABEL = os.environ.get("B5A_CASE_LABEL", "h0_v0")


def _fail(msg: str):
    raise RuntimeError("[B5A multiset] " + msg)
...
def _target_from_df_or_env(df: pd.DataFrame, row: pd.Series):
    ...
    lon_env = _env_angle_rad("B5A_TARGET_LON_DEG", "B5A_TARGET_LON_RAD")
    lat_env = _env_angle_rad("B5A_TARGET_LAT_DEG", "B5A_TARGET_LAT_RAD")
    if lon_env is not None and lat_env is not None:
        return lon_env, lat_env, "environment target"

    _fail("Could not infer target lon/lat from CSV. Set B5A_TARGET_LON_DEG/LAT_DEG.")
...
# ------------------------------------------------------------
# Role specs: heading offsets are controlled by B5A_HEADING_SPREAD_DEG.
# ------------------------------------------------------------
_ROLE_SPECS = [
    dict(
        vehicle_id=0,
        role_name="B5A_mis0_left_heading_probe",
        phase2_role="B5A_CLASSIFY_MIS0_LEFT_HEADING_PROBE",
        heading_delta_deg=-B5A_HEADING_SPREAD_DEG,
        pathspec=dict(
            name="B5A_mis0_left_fixed_anchor_probe",
    ...
    dict(
        ...
        phase2_role="B5A_REFERENCE_MIS1_B1E_CONTROL",
        heading_delta_deg=0.0,
        pathspec=dict(
            name="B5A_mis1_v8_2_validated_nominal",
    ...
    dict(
        ...
        phase2_role="B5A_CLASSIFY_MIS2_RIGHT_HEADING_PROBE",
        heading_delta_deg=+B5A_HEADING_SPREAD_DEG,
        pathspec=dict(
            name="B5A_mis2_right_case2_probe",
            policy_family="merged_winner_case4_case2",
            psi_bias_deg=-0.5,
            psi_bias_sgo_on_m=7.0e6,
...
def _make_case(spec: dict):
    ...
    pathspec.update(
        ...
        b5a_heading_spread_deg=float(B5A_HEADING_SPREAD_DEG),
        b5a_height_delta_m=float(B5A_HEIGHT_DELTA_M),
        b5a_velocity_delta_mps=float(B5A_VELOCITY_DELTA_MPS),
        b5a_case_label=B5A_CASE_LABEL,
        baseline_csv=BASELINE_CSV,
        target_source=_TARGET_SOURCE,
        b5a_vehicle_id=int(spec["vehicle_id"]),
    )
    ...
    return {
        'MissileInitStatus': {
            ...
            "longitude": _INIT_LON,
            "latitude": _INIT_LAT,
            "height": max(1000.0, _INIT_H + B5A_HEIGHT_DELTA_M),
            "velocity": max(100.0, _INIT_V + B5A_VELOCITY_DELTA_MPS),
        ...
        'MissileEndStatus': {
            ...
            "s": _STAEM,
            "heading_angle": None,
            "t": float(os.environ.get("B5A_END_T", os.environ.get("B2R_END_T", "2150"))),
        },
        'PathSpec': pathspec,
        'Phase2Role': spec["phase2_role"],
        'RoleName': spec["role_name"],
        'RoleHeadingDeltaDeg': float(spec["heading_delta_deg"]),
        'B5AHeadingSpreadDeg': float(B5A_HEADING_SPREAD_DEG),
        'B5AHeightDeltaM': float(B5A_HEIGHT_DELTA_M),
        'B5AVelocityDeltaMps': float(B5A_VELOCITY_DELTA_MPS),
        'B5ACaseLabel': B5A_CASE_LABEL,
    }


StatusParams = [_make_case(s) for s in _ROLE_SPECS]
cases = StatusParams
case_num = int(float(os.environ.get("B5A_DEFAULT_CASE_NUM", "1")))
...
B5A_VEHICLE_IDS = [0, 1, 2]
B5A_BASELINE_CSV_USED = BASELINE_CSV
B5A_TARGET_SOURCE_USED = _TARGET_SOURCE
B5A_ROLE_SPECS = _ROLE_SPECS
...
MAX_EBR2_ITER = int(float(os.environ.get("B5A_MAX_EBR2_ITER", os.environ.get("B2R_MAX_EBR2_ITER", "5"))))
STORE_DATA = os.environ.get("B5A_STORE_DATA", os.path.join("store", "data_saved", "phase2_B5A_heading_spread_combined.csv"))

print("[B5A multiset] baseline_csv =", BASELINE_CSV)
print("[B5A multiset] heading_spread_deg =", B5A_HEADING_SPREAD_DEG)
print("[B5A multiset] height_delta_m =", B5A_HEIGHT_DELTA_M)
print("[B5A multiset] velocity_delta_mps =", B5A_VELOCITY_DELTA_MPS)
print("[B5A multiset] case_label =", B5A_CASE_LABEL)
print("[B5A multiset] target lon/lat deg =", np.rad2deg(_TAR_LON), np.rad2deg(_TAR_LAT), "source=", _TARGET_SOURCE)
print("[B5A multiset] base init lon/lat/heading deg =", np.rad2deg(_INIT_LON), np.rad2deg(_INIT_LAT), np.rad2deg(_INIT_PSI_BASE))
print("[B5A multiset] end h/v/s =", _HTAEM, _VTAEM, _STAEM)
for _i, _case in enumerate(StatusParams):
    _ps = _case["PathSpec"]
    print(
        f"[B5A multiset] StatusParams[{_i}] spread={B5A_HEADING_SPREAD_DEG} role={_case.get('Phase2Role')} "

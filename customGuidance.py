...
class CustomGuidance(Guidance):
    def __init__(self):
        ...
        self.alpha_2 = np.deg2rad(getattr(glbs, "ALPHA_2_DEG", 8.0))
        self.V_TAEM = 0  # 终端速度
        self.E_TAEM = 0  # 终端能量
        self.H_TAEM = 0  # 终端高度
        self.E_alpha = -5.55e7
    ...
    def attack_angle_plan(self, E):
        # fsolve/NumPy zincirinden numpy.float64 gelebilir; skaler float'a çevir.
        E = float(np.asarray(E).reshape(-1)[0])

        a1 = self.alpha_sgp
        a2 = self.alpha_2
        # E >= E_alpha ise doğrudan a1
        if E >= self.E_alpha:
            return a1

        denom = (self.E_alpha - self.E_TAEM)
        if denom == 0:
            return a1

        # Normalize parametre (0..1 aralığına sıkıştır)
        tau = (self.E_alpha - E) / denom
        tau = float(np.clip(tau, 0.0, 1.0))

        return (tau ** 2) * (a2 - a1) + a1

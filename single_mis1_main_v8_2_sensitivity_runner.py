...
def main():
    ...
    parser.add_argument("--dwell-count", type=int, default=3, help="Required consecutive in-box samples for TAEM success")
    parser.add_argument("--dwell-seconds", type=float, default=None, help="Required in-box dwell seconds; default equals dwell-count")
    parser.add_argument("--h-tol", type=float, default=2500.0, help="TAEM entry height tolerance [m]")
    parser.add_argument("--v-tol", type=float, default=130.0, help="TAEM entry velocity tolerance [m/s]")
    parser.add_argument("--sgo-tol", type=float, default=25000.0, help="TAEM entry s_go tolerance [m]")
    parser.add_argument("--out", default=None, help="Output CSV path; default auto-names by dwell/tolerances")
    args = parser.parse_args()
    if args.dwell_seconds is None:
        args.dwell_seconds = float(args.dwell_count)
    if args.out is None:
        tag = f"dwellN{args.dwell_count}_h{int(args.h_tol)}_v{int(args.v_tol)}_sgo{int(args.sgo_tol)}"
        args.out = os.path.join("store", "data_saved", f"mis1_state_first_v8_2_{tag}.csv")
    ...
    # Sensitivity overrides: keep V8.2 dynamics unchanged, alter only TAEM success criteria.
    guide.taem_dwell_required_count = int(args.dwell_count)
    guide.taem_dwell_required_s = float(args.dwell_seconds)
    guide.taem_tol_h_enter_m = float(args.h_tol)
    guide.taem_tol_v_enter_mps = float(args.v_tol)
    guide.taem_tol_sgo_enter_m = float(args.sgo_tol)
    # Exit hysteresis kept wider than entry to avoid one-sample flicker.
    guide.taem_tol_h_exit_m = max(float(args.h_tol) * 1.68, float(args.h_tol) + 1000.0)
    guide.taem_tol_v_exit_mps = max(float(args.v_tol) * 1.46, float(args.v_tol) + 50.0)
    guide.taem_tol_sgo_exit_m = max(float(args.sgo_tol) * 1.60, float(args.sgo_tol) + 10000.0)
    ...
    print(f"[INFO] dwell_count={args.dwell_count}, dwell_seconds={args.dwell_seconds}, h_tol={args.h_tol}, v_tol={args.v_tol}, sgo_tol={args.sgo_tol}")
    if len(result):

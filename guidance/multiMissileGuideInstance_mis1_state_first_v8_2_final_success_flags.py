...
class Mis1StateFirstGuidanceV8Final(Mis1StateFirstGuidance):
    """V8.2 final-success wrapper over the only run family that produced a real TAEM dwell.
    ...
    def init(self, missile, target=None, meta=None):
        ...
        missile.guide.update({
            ...
            "success_mode": "v8_2_final_success_flags",
        })

    def update_EBR2(self, mis, tar=None, meta=None):
        """Safe wrapper for inherited online EBR2 update.

        The inherited CustomGuidance.update_EBR2() uses a secant denominator
        (vf_new - vf_old). In tolerance-sensitivity runs, this denominator can
        become zero and abort the run. Preserve nominal behavior when it is
        well-conditioned; skip only the singular online update.
        """
        if meta is None:
            meta = {}
        try:
            return super().update_EBR2(mis, tar, meta)
        except (FloatingPointError, ZeroDivisionError) as exc:
            print(f"[V8.2 safe] Skipping singular online EBR2 update at t={meta.get('t', None)}: {exc}")
            mis.guide["ebr2_update_skipped"] = True
            mis.guide["ebr2_update_skip_reason"] = "singular_secant_denominator"
            return None
    ...
    def _latch_success_if_needed(self, mis, meta=None):
        ...
        # V8.2 policy: keep the explicit event flag TRUE on the final saved row.
        # Some compare scripts prioritize taem_reached_event over taem_reached_ever;
        # a one-step pulse can be lost when guide/end/final-save are called in the
        # same simulation cycle. For final reporting we persist the event after latch.
        first_event = not was_latched
        mis.guide["taem_reached_event"] = True
